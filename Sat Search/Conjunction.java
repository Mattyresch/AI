...
public class Conjunction {
	...
	public static SearchControl superFancySearch = new SearchControl(){
		...
		public Assignment pick (Conjunction c){
			return new Assignment(c.mostFrequentVariable().v, new Boolean(true));}
	...
	SearchResult search(SearchControl control, SearchStats stats, int depth) {
		//System.out.println(this.assignments.toString());
		//System.out.println(this.variables.toString());
		//**************************************************
		//* Use this block of code to check for consistency*
		//**************************************************
		stats.expand(depth);
	    ...
	    if(this.isConsistent()){
	    	stats.succeed(depth);
	    	return new SearchResult(this.assignments);
	    }else if(this.isInconsistent()){
	    	stats.deadEnd(depth);
	    	//if(control.backjump() == true){
	    	//	return new SearchResult(this.conflictInducedClause());
	    	//}
	    	return new SearchResult(SearchStatus.Failure);
	    }
	    //***********************************************
	    //* Use this block to run a search with new vars*
	    //* value picked from the queue of vars. Default*
	    //* value for var is true.                      *
	    //***********************************************
		Assignment a = control.pick(this);
		//this.assign(a.v, a.b, null);
		this.assign(a.v, a.b, this.variables);
		this.justifications.put(a.v, this.variables);
		SearchResult s1 = this.search(control, stats, depth++);
	    if(s1.getStatus() == SearchStatus.Success){
	    	stats.succeed(depth);
	    	return new SearchResult(this.assignments);
	    }else if(s1.getStatus() == SearchStatus.Timeout){
	    	stats.fail();
	    	System.out.println("We ran out of time!\n");
	    	return new SearchResult(SearchStatus.Timeout);
	    }
	    else{
	    	//***************************************************
	    	//* Use this block to run another search if the 1st *
	    	//* Search failed on the 1st var value.             *
	    	//***************************************************
	    	System.out.println("There wasn't a left-branch match for " + a.v + " at depth " + depth++ +" for the value " + a.b.toString() +"."  );
	    	//System.out.println("Disjunction for left branch: " + s1.getConflictInducedClause().toString());
	    	this.assign(a.v, !a.b, this.variables);
	    	SearchResult s2 = null;
	    	s2 = this.search(control, stats, depth++);
	    	//System.out.println(s2.toString());
	    	if(s2.getStatus() == SearchStatus.Success){
	    		//**************************************************
	    		//* Dope, there was a successful branch found with *
	    		//* the new variable value. Report it!             *
	    		//**************************************************
		    	System.out.println("There was a right-branch match for " + a.v + " at depth " + depth++ +" for the value " + a.b.toString() +"."  );
	    		stats.succeed(depth);
	    		return s2;
	    	}else if(s2.getStatus() == SearchStatus.Failure){
	    		//*********************************************************
	    		//* Uh oh, we didn't find a single success. What do       *
	    		//* we do here? Report dead end, resolve the disjunctions *
	    		//* and return Search Result with that disjunction.       *
	    		//*********************************************************
		    	System.out.println("There wasn't a right-branch match for " + a.v + " at depth " + depth++ +" for the value " + a.b.toString() +". Dead end."  );
	    		stats.deadEnd(depth);
	    		/*
	    		 * if(control.backjump() == true){
	    			Disjunction d = new Disjunction();
	    			d = s1.getConflictInducedClause();
	    			Disjunction d2 = new Disjunction();
	    			d2 = s2.getConflictInducedClause();
	    			Disjunction r = null;
	    			r = Disjunction.resolve(d, d2);
	...
	public static void main(String[] args) throws IOException {
	    ...
	    String o = doSearchWithTimeLimit("tests/simple_v3_c2.cnf", superFancySearch, 1000L);
		//String o2 = doSearchWithTimeLimit("tests/aim-50-1_6-yes1-4.cnf", superFancySearch, 1000L);
	    //String o3 = doSearchWithTimeLimit("tests/aim-100-1_6-no-1.cnf", vanillaSearch, 1000L);
